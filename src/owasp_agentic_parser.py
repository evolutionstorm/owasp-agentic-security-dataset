#!/usr/bin/env python3
"""
OWASP Top 10 for Agentic Applications 2026 - Parser and Data Extractor
Converts the OWASP Agentic Top 10 to machine-readable JSON and YAML formats.

Author: Generated for AgentGuard Architect
...
def create_owasp_agentic_top10() -> Dict[str, Any]:
    """
    Creates the complete OWASP Agentic Top 10 data structure.
    """
    ...
    data = {
        "metadata": {
            ...
            "description": "The OWASP Top 10 for Agentic Applications serves as a compass and go-to reference for security leaders and practitioners to understand and address the Top 10 highest-impact threats for agentic AI systems."
        },
        "entries": create_entries(),
        "mappings": create_mappings(),
        "nhi_mappings": create_nhi_mappings(),
        "related_frameworks": {
            "owasp_llm_top10": {
                "version": "2025",
            ...
            "agentic_threats_mitigations": {
                "version": "1.1",
                "description": "OWASP Agentic AI Threats and Mitigations Guide"
            },
            "aivss": {
                "name": "OWASP AI Vulnerability Scoring System",
                "description": "Scoring framework for AI vulnerabilities"
            },
            "cyclonedx": {
                "description": "OWASP CycloneDX - Bill of Materials standard"
            }
        }
    }
...
def create_entries() -> List[Dict[str, Any]]:
    """
    Creates all 10 ASI entries with complete details.
    """
    ...
    entries = [
        {
            ...
            "common_examples": [
                ...
                "Indirect Prompt Injection through external communication channels (e.g. email, calendar, teams) sent from outside of the company hijacks an agent's internal communication capability",
            ...
            "attack_scenarios": [
                {
                    ...
                    "description": "An attacker emails a crafted message that silently triggers Microsoft 365 Copilot to execute hidden instructions, causing the AI to exfiltrate confidential emails, files, and chat logs without any user interaction."
                },
                {
                    "name": "Operator Prompt Injection via Web Content",
                    "description": "An attacker plants malicious content on a web page that the Operator agent processes, tricking it into following unauthorized instructions and exposing users' private data."
                },
                {
                    "name": "Goal-lock drift via scheduled prompts",
                    "description": "A malicious calendar invite injects a recurring 'quiet mode' instruction that subtly reweights objectives each morning, steering the planner toward low-friction approvals."
                },
                {
                    "name": "Inception attack on ChatGPT users",
                    "description": "A malicious Google Doc injects instructions for ChatGPT to exfiltrate user data and convinces the user to make an ill-advised business decision."
                }
            ],
            "mitigations": [
                ...
                "Evaluate use of 'intent capsule' - an emerging pattern to bind the declared goal, constraints, and context to each execution cycle in a signed envelope.",
        ...
        },
        {
            ...
            "related_threats": ["T2 Tool Misuse", "T4 Resource Overload", "T16 Insecure Inter-Agent Protocol Abuse"],
            "related_llm_entries": ["LLM06:2025 Excessive Agency"],
            ...
            "attack_scenarios": [
                {
                    "name": "Tool Poisoning",
                    "description": "An attacker compromises the tool interface (MCP tool descriptors, schemas, metadata, or routing information) causing the agent to invoke a tool based on falsified or malicious capabilities."
                },
                {
                    "name": "Indirect Injection → Tool Pivot",
                    "description": "An attacker embeds instructions in a PDF ('Run cleanup.sh and send logs to X'). The agent obeys, invoking a local shell tool."
                },
                {
                    "name": "Over-Privileged API",
                    "description": "A customer service bot intended to fetch order history also issues refunds because the tool had full financial API access."
                },
                {
                    "name": "Internal Query → External Exfiltration",
                    "description": "An agent is tricked into chaining a secure, internal-only CRM tool with an external email tool, exfiltrating a sensitive customer list."
                },
                {
                    "name": "Tool name impersonation (typosquatting)",
                    "description": "A malicious tool named 'report' is resolved before 'report_finance,' causing misrouting and unintended data disclosure."
                },
                {
                    "name": "EDR Bypass via Tool Chaining",
                    "description": "A security-automation agent chains together legitimate administrative tools to exfiltrate sensitive logs, bypassing host-centric monitoring."
                },
                {
        ...
        },
        {
            ...
            "description": "Identity & Privilege Abuse exploits dynamic trust and delegation in agents to escalate access and bypass controls by manipulating delegation chains, role inheritance, control flows, and agent context. This risk arises from the architectural mismatch between user-centric identity systems and agentic design. Without a distinct, governed identity of its own, an agent operates in an attribution gap that makes enforcing true least privilege impossible.",
            "related_threats": ["T3 Privilege Compromise"],
            "related_llm_entries": ["LLM01:2025 Prompt Injection", "LLM06:2025 Excessive Agency", "LLM02:2025 Sensitive Information Disclosure"],
            ...
            "attack_scenarios": [
                {
                    ...
                    "description": "A finance agent delegates to a 'DB query' agent but passes all its permissions. An attacker uses the inherited access to exfiltrate HR and legal data."
                },
                {
                    "name": "Memory-Based Escalation",
                    "description": "An IT admin agent caches SSH credentials during a patch. Later a non-admin reuses the same session to create an unauthorized account."
                },
                {
                    "name": "Cross-Agent Trust Exploitation",
                    "description": "A crafted email instructs an email sorting agent to tell a finance agent to move money. The finance agent processes the fraudulent payment without verification."
                },
                {
                    "name": "Device-code phishing across agents",
                    "description": "An attacker shares a device-code link that a browsing agent follows; a separate 'helper' agent completes the code, binding the victim's tenant to attacker scopes."
                },
                {
                    "name": "Forged Agent Persona",
                    "description": "An attacker registers a fake 'Admin Helper' agent in an internal Agent2Agent registry with a forged agent card."
                },
                {
        ...
        },
        {
            ...
            "description": "Agentic Supply Chain Vulnerabilities arise when agents, tools, and related artefacts they work with are provided by third parties and may be malicious, compromised, or tampered with in transit. Unlike traditional AI or software supply chains, agentic ecosystems often compose capabilities at runtime - loading external tools and agent personas dynamically – thereby increasing the attack surface.",
            "related_threats": ["T17 Supply Chain Compromise", "T2 Tool Misuse", "T11 Unexpected RCE", "T12 Agent Communication Poisoning", "T13 Rogue Agent", "T16 Insecure Inter-Agent Protocol Abuse"],
            "related_llm_entries": ["LLM03:2025 Supply Chain Vulnerabilities"],
            ...
            "attack_scenarios": [
                {
                    ...
                    "description": "A poisoned prompt in the Q for VS Code repo ships in v1.84.0 to thousands before detection, showing how upstream agent-logic tampering cascades."
                },
                {
                    "name": "MCP Tool Descriptor Poisoning",
                    "description": "A researcher shows a prompt injection in GitHub's MCP where a malicious public tool hides commands in its metadata."
                },
                {
                    "name": "Malicious MCP Server Impersonating Postmark",
                    "description": "First in-the-wild malicious MCP server on npm, impersonating postmark-mcp and secretly BCC'ing emails to the attacker."
                },
                {
                ...
                },
                {
                    ...
                    "description": "A poisoned nx/debug release was automatically installed by coding agents, enabling hidden backdoor that exfiltrated SSH keys."
                },
                {
                    "name": "Agent-in-the-Middle via Agent Cards",
                    "description": "A compromised peer advertises exaggerated capabilities in its agent card, intercepting privileged coordination traffic."
                }
            ],
            "mitigations": [
        ...
        },
        {
            ...
            "related_threats": ["T11 Unexpected RCE and Code Attacks"],
            "related_llm_entries": ["LLM01:2025 Prompt Injection", "LLM05:2025 Improper Output Handling"],
            ...
            "attack_scenarios": [
                {
                    ...
                    "description": "During automated self-repair tasks, an agent generates and executes unreviewed install or shell commands, deleting or overwriting production data."
                },
                {
                    "name": "Direct Shell Injection",
                    "description": "An attacker submits a prompt with embedded shell commands disguised as legitimate instructions, resulting in unauthorized system access."
                },
                {
                ...
                },
                {
                    ...
                    "description": "An agent generates a serialized object containing malicious payload data that triggers code execution when deserialized."
                },
                {
                    "name": "Multi-Tool Chain Exploitation",
                    "description": "An attacker crafts a prompt causing the agent to invoke tools in sequence (file upload → path traversal → dynamic code loading)."
                },
                {
                    "name": "Memory System RCE",
                    "description": "An attacker exploits an unsafe eval() function in the agent's memory system by embedding executable code within prompts."
                },
                {
                    "name": "Agent-Generated RCE",
                    "description": "An agent trying to patch a server is tricked into downloading and executing a vulnerable package for a reverse shell."
                },
                {
                    "name": "Dependency lockfile poisoning",
                    "description": "The agent regenerates a lockfile from unpinned specs and pulls a backdoored minor version during 'fix build' tasks."
                }
            ],
            "mitigations": [
                "Follow LLM05:2025 Improper Output Handling mitigations with input validation and output encoding.",
                "Prevent direct agent-to-production systems with pre-production checks including security evaluations and adversarial unit tests.",
        ...
        },
        {
            ...
            "related_threats": ["T1 Memory Poisoning", "T4 Memory Overload", "T6 Broken Goals", "T12 Shared Memory Poisoning"],
            "related_llm_entries": ["LLM01:2025 Prompt Injection", "LLM04:2025 Data and Model Poisoning", "LLM08:2025 Vector and Embedding Weaknesses"],
            ...
            "attack_scenarios": [
                {
                    ...
                    "description": "An attacker keeps reinforcing a fake flight price, the assistant stores it as truth, then approves bookings at that price."
                },
                {
                    "name": "Context Window Exploitation",
                    "description": "The attacker splits attempts across sessions so earlier rejections drop out of context, eventually getting admin access."
                },
                {
                ...
                },
                {
                    ...
                    "description": "Near-duplicate content seeded by attacker exploits loose namespace filters, pulling another tenant's sensitive chunk."
                },
                {
                    "name": "Assistant Memory Poisoning",
                    "description": "An attacker implants a user assistant's memory via Indirect Prompt Injection, compromising current and future sessions."
                }
            ],
            "mitigations": [
        ...
        },
        {
            ...
            "description": "Multi agent systems depend on continuous communication between autonomous agents that coordinate via APIs, message buses, and shared memory, significantly expanding the attack surface. Insecure Inter-Agent Communication occurs when these exchanges lack proper authentication, integrity, or semantic validation - allowing interception, spoofing, or manipulation of agent messages and intents.",
            "related_threats": ["T12 Agent Communication Poisoning", "T16 Insecure Inter-Agent Protocol Abuse"],
            "related_llm_entries": ["LLM02:2025 Sensitive Information Disclosure", "LLM06:2025 Excessive Agency"],
        ...
        },
        {
            ...
            "related_threats": ["T5 Cascading Hallucination Attacks", "T8 Repudiation and Untraceability"],
            "related_llm_entries": ["LLM01:2025 Prompt Injection", "LLM04:2025 Data and Model Poisoning", "LLM06:2025 Excessive Agency"],
            ...
            "attack_scenarios": [
                {
                    ...
                    "description": "Prompt injection poisons a Market Analysis agent, inflating risk limits; Position and Execution agents auto-trade larger positions."
                },
                {
                    "name": "Healthcare protocol propagation",
                    "description": "Supply chain tampering corrupts drug data; Treatment auto-adjusts protocols, Care Coordination spreads them network-wide."
                },
                {
                    "name": "Cloud orchestration breakdown",
                    "description": "Poisoning in Resource Planning adds unauthorized permissions; Security applies them, Deployment provisions backdoored infrastructure."
                },
                {
        ...
        },
        {
            ...
            "description": "Intelligent agents can establish strong trust with human users through their natural language fluency, emotional intelligence, and perceived expertise (anthropomorphism). Adversaries or misaligned designs may exploit this trust to influence user decisions, extract sensitive information, or steer outcomes for malicious purposes. The agent acts as an untraceable 'bad influence.'",
            "related_threats": ["T7 Misaligned & Deceptive Behaviors", "T8 Repudiation & Untraceability", "T10 Overwhelming the Human in the Loop"],
            "related_llm_entries": ["LLM01:2025 Prompt Injection", "LLM05:2025 Improper Output Handling", "LLM06:2025 Excessive Agency", "LLM09:2025 Misinformation"],
            ...
            "attack_scenarios": [
                ...
                },
                {
                    ...
                    "description": "A prompt-injected IT support agent targets a new hire, cites real tickets to appear legit, then captures credentials."
                },
                {
                    "name": "Invoice Copilot Fraud",
                    "description": "A poisoned vendor invoice is ingested by the finance copilot which suggests urgent payment to attacker bank details."
                },
                {
                ...
                },
                {
                    ...
                    "description": "A hijacked agent fabricates a convincing rationale to trick an analyst into approving deletion of a live database."
                },
                {
        ...
        },
        {
            ...
            "related_threats": ["T13 Rogue Agents in Multi-Agent Systems", "T14 Human Attacks on Multi-Agent Systems", "T15 Human Manipulation"],
            "related_llm_entries": ["LLM02:2025 Sensitive Information Disclosure", "LLM09:2025 Misinformation"],
            ...
            "attack_scenarios": [
                {
                    ...
                    "description": "After encountering a poisoned web instruction, the agent continues independently scanning and transmitting sensitive files even after the malicious source is removed."
                },
                {
                    "name": "Impersonated Observer Agent (Integrity Violation)",
                    "description": "An attacker injects a fake review or approval agent into a multi-agent workflow, misleading a payment processing agent."
                },
                {
...
def create_mappings() -> Dict[str, Any]:
    """
    Creates the mapping matrix between ASI, LLM Top 10, Threats & Mitigations, and AIVSS.
    """
    
    return {
        "description": "Cross-Mapping the ASI Top 10, LLM Top 10, Agentic AI Threats & Mitigations, and AIVSS Core Risks",
        "matrix": [
            {
                "asi_id": "ASI01",
                "llm_top10": ["LLM01:2025 Prompt Injection", "LLM06:2025 Excessive Agency"],
                "threats_mitigations": ["T6 Goal Manipulation", "T7 Misaligned & Deceptive Behaviors"],
                "aivss_core_risk": "Agent Goal & Instruction Manipulation"
            },
            {
                "asi_id": "ASI02",
                "llm_top10": ["LLM06:2025 Excessive Agency"],
                "threats_mitigations": ["T2 Tool Misuse", "T4 Resource Overload", "T16 Insecure Inter-Agent Protocol Abuse"],
                "aivss_core_risk": "Agentic AI Tool Misuse"
            },
            {
                "asi_id": "ASI03",
                "llm_top10": ["LLM01:2025 Prompt Injection", "LLM06:2025 Excessive Agency", "LLM02:2025 Sensitive Info Disclosure"],
                "threats_mitigations": ["T3 Privilege Compromise"],
                "aivss_core_risk": "Agent Access Control Violation"
            },
            {
                "asi_id": "ASI04",
                "llm_top10": ["LLM03:2025 Supply Chain Vulnerabilities"],
                "threats_mitigations": ["T17 Supply Chain Compromise", "T2 Tool Misuse", "T11 Unexpected RCE", "T12 Agent Comm Poisoning", "T13 Rogue Agent", "T16 Insecure Inter-Agent Protocol Abuse"],
                "aivss_core_risk": "Agent Supply Chain & Dependency Attacks"
            },
            {
                "asi_id": "ASI05",
                "llm_top10": ["LLM01:2025 Prompt Injection", "LLM05:2025 Improper Output Handling"],
                "threats_mitigations": ["T11 Unexpected RCE & Code Attacks"],
                "aivss_core_risk": "Insecure Agent Critical Systems Interaction"
            },
            {
                "asi_id": "ASI06",
                "llm_top10": ["LLM01:2025 Prompt Injection", "LLM04:2025 Data & Model Poisoning", "LLM08:2025 Vector & Embedding Weaknesses"],
                "threats_mitigations": ["T1 Memory Poisoning", "T4 Memory Overload", "T6 Broken Goals", "T12 Shared Memory Poisoning"],
                "aivss_core_risk": "Memory Use & Contextual Awareness"
            },
            {
                "asi_id": "ASI07",
                "llm_top10": ["LLM02:2025 Sensitive Information Disclosure", "LLM06:2025 Excessive Agency"],
                "threats_mitigations": ["T12 Agent Communication Poisoning", "T16 Insecure Inter-Agent Protocol Abuse"],
                "aivss_core_risk": "Agent Memory & Context Manipulation"
            },
            {
                "asi_id": "ASI08",
                "llm_top10": ["LLM01:2025 Prompt Injection", "LLM04:2025 Data & Model Poisoning", "LLM06:2025 Excessive Agency"],
                "threats_mitigations": ["T5 Cascading Hallucination Attacks", "T8 Repudiation & Untraceability"],
                "aivss_core_risk": "Agent Cascading Failures"
            },
            {
                "asi_id": "ASI09",
                "llm_top10": ["LLM01:2025 Prompt Injection", "LLM05:2025 Improper Output Handling", "LLM06:2025 Excessive Agency", "LLM09:2025 Misinformation"],
                "threats_mitigations": ["T7 Misaligned & Deceptive Behaviors", "T8 Repudiation & Untraceability", "T10 Overwhelming Human in the Loop"],
                "aivss_core_risk": "Agent Untraceability / Human Manipulation"
            },
            {
                "asi_id": "ASI10",
                "llm_top10": ["LLM02:2025 Sensitive Information Disclosure", "LLM09:2025 Misinformation"],
                "threats_mitigations": ["T13 Rogue Agents in Multi-Agent Systems", "T14 Human Attacks on Multi-Agent Systems", "T15 Human Manipulation"],
                "aivss_core_risk": "Behavioral Integrity (BI), Operational Security (OS), Compliance Violations (CV)"
            }
        ]
    }
...
def create_nhi_mappings() -> Dict[str, Any]:
    """
    Creates mapping between OWASP Non-Human Identities Top 10 and OWASP Agentic AI Top 10.
    """
    
    return {
        "description": "Mapping Between OWASP Non-Human Identities Top 10 (2025) and OWASP Agentic AI Top 10",
        "mappings": [
            {
                "nhi_id": "NHI1",
                "nhi_title": "Improper Offboarding",
                "description": "Failure to deactivate or remove unused non-human identities leads to persistent attack surface.",
                "asi_mappings": ["ASI04 – Agentic Supply Chain Vulnerabilities"],
                "threats": ["T17 Supply Chain Compromise", "T2 Tool Misuse"],
                "aivss": "Agent Supply Chain & Dependency Attacks"
            },
            {
                "nhi_id": "NHI2",
                "nhi_title": "Secret Leakage",
                "description": "Exposure of API keys, tokens, or certificates used by non-human identities.",
                "asi_mappings": ["ASI02 – Tool Misuse & Exploitation", "ASI06 – Memory & Context Poisoning"],
                "threats": ["T6 Goal Manipulation", "T1 Memory Poisoning"],
                "aivss": "Memory Use & Contextual Awareness"
            },
            {
                "nhi_id": "NHI3",
                "nhi_title": "Vulnerable Third-Party NHI",
                "description": "Third-party integrated identities are compromised and exploited.",
                "asi_mappings": ["ASI04 – Supply Chain Vulnerabilities", "ASI03 – Identity & Privilege Abuse"],
                "threats": ["T12 Agent Communication Poisoning", "T13 Rogue Agents"],
                "aivss": "Agent Supply Chain & Dependency Attacks"
            },
            {
                "nhi_id": "NHI4",
                "nhi_title": "Insecure Authentication",
                "description": "Weak or deprecated authentication mechanisms for NHIs.",
                "asi_mappings": ["ASI03 – Identity & Privilege Abuse", "ASI07 – Insecure Inter-Agent Communication"],
                "threats": ["T16 Insecure Inter-Agent Protocol Abuse"],
                "aivss": "Agent Access Control Violation"
            },
            {
                "nhi_id": "NHI5",
                "nhi_title": "Overprivileged NHI",
                "description": "Non-human identities granted excessive permissions.",
                "asi_mappings": ["ASI02 – Tool Misuse & Exploitation", "ASI03 – Identity & Privilege Abuse"],
                "threats": ["T2 Tool Misuse", "T3 Privilege Compromise"],
                "aivss": "Agent Access Control Violation"
            },
            {
                "nhi_id": "NHI6",
                "nhi_title": "Insecure Cloud Deployment Configurations",
                "description": "Misconfigured CI/CD and cloud setups with static credentials.",
                "asi_mappings": ["ASI04 – Supply Chain Vulnerabilities", "ASI05 – Unexpected Code Execution (RCE)"],
                "threats": ["T11 Unexpected RCE & Code Attacks"],
                "aivss": "Insecure Agent Critical Systems Interaction"
            },
            {
                "nhi_id": "NHI7",
                "nhi_title": "Long-Lived Secrets",
                "description": "Credentials or keys with long expiry increase attacker dwell time.",
                "asi_mappings": ["ASI06 – Memory & Context Poisoning", "ASI08 – Cascading Failures"],
                "threats": ["T4 Memory Overload", "T12 Shared Memory Poisoning"],
                "aivss": "Memory Use & Contextual Awareness"
            },
            {
                "nhi_id": "NHI8",
                "nhi_title": "Environment Isolation",
                "description": "Reuse of NHIs across dev/test/prod enabling lateral movement.",
                "asi_mappings": ["ASI08 – Cascading Failures", "ASI07 – Insecure Inter-Agent Communication"],
                "threats": ["T8 Repudiation & Untraceability", "T12 Agent Communication Poisoning"],
                "aivss": "Agent Cascading Failures"
            },
            {
                "nhi_id": "NHI9",
                "nhi_title": "NHI Reuse",
                "description": "Reusing same NHI across services amplifies impact of compromise.",
                "asi_mappings": ["ASI08 – Cascading Failures", "ASI04 – Supply Chain Vulnerabilities"],
                "threats": ["T5 Cascading Hallucination Attacks", "T13 Rogue Agents"],
                "aivss": "Agent Cascading Failures"
            },
            {
                "nhi_id": "NHI10",
                "nhi_title": "Human Use of NHI",
                "description": "Humans using non-human credentials causes loss of accountability and privilege misuse.",
                "asi_mappings": ["ASI09 – Human-Agent Trust Exploitation", "ASI01 – Agent Goal Hijack"],
                "threats": ["T10 Overwhelming Human in the Loop", "T7 Misaligned & Deceptive Behaviors"],
                "aivss": "Agent Untraceability / Human Manipulation"
            }
        ]
    }


def create_exploits_tracker() -> List[Dict[str, Any]]:
    """
    Creates the exploits and incidents tracker data.
    """
    
    return [
        {
            "date": "2025-10",
            "name": "Malicious MCP Package Backdoor",
            "impact": "NPM package hosted backdoored MCP server with dual reverse shells, giving persistent remote access to agent environments.",
            "asi_mappings": ["ASI04"],
            "references": ["NPM", "Koi Security"]
        },
        {
            "date": "2025-10",
            "name": "Framelink Figma MCP RCE",
            "impact": "Unsanitized user input in Framelink Figma MCP's get_figma_data tool enabled unauthenticated remote command execution.",
            "asi_mappings": ["ASI05", "ASI02"],
            "references": ["Figma Context MCP", "NVD", "Imperva"]
        },
        {
            "date": "2025-09",
            "name": "Google Gemini Trifecta",
            "impact": "Indirect prompt injection through logs, search history, and browsing context can trick Gemini into exposing sensitive data.",
            "asi_mappings": ["ASI01", "ASI02"],
            "references": ["Tenable"]
        },
        {
            "date": "2025-09",
            "name": "Malicious MCP Server Impersonating Postmark",
            "impact": "First in-the-wild malicious MCP server on npm; impersonated postmark-mcp and secretly BCC'd emails to the attacker.",
            "asi_mappings": ["ASI02", "ASI04", "ASI07"],
            "references": ["Postmark", "Koi Security"]
        },
        {
            "date": "2025-09",
            "name": "ForcedLeak (Salesforce Agentforce)",
            "impact": "Critical indirect prompt injection in Salesforce Agentforce allows an external attacker to exfiltrate sensitive CRM records.",
            "asi_mappings": ["ASI01", "ASI02"],
            "references": ["Salesforce", "Noma Security"]
        },
        {
            "date": "2025-07",
            "name": "Amazon Q Prompt Poisoning",
            "impact": "Destructive prompt in extension risked file wipes.",
            "asi_mappings": ["ASI01", "ASI02", "ASI04"],
            "references": ["AWS", "NVD"]
        },
        {
            "date": "2025-07",
            "name": "Replit Vibe Coding Meltdown",
            "impact": "Agent hallucinated data, deleted a production DB, and generated false outputs to hide mistakes.",
            "asi_mappings": ["ASI01", "ASI09", "ASI10"],
            "references": ["Replit", "SaaStr"]
        },
        {
            "date": "2025-05",
            "name": "EchoLeak (Zero-Click Prompt Injection)",
            "impact": "Critical zero-click exploit allowing a mere email to trigger Copilot into leaking confidential data.",
            "asi_mappings": ["ASI01", "ASI02", "ASI06"],
            "references": ["Microsoft", "NVD", "Aim Security"]
        },
        {
            "date": "2025-04",
            "name": "Agent-in-the-Middle (A2A Protocol Spoofing)",
            "impact": "A malicious agent published a fake agent card in an open A2A directory, enabling interception of sensitive data.",
            "asi_mappings": ["ASI03", "ASI06", "ASI07", "ASI08", "ASI10"],
            "references": ["Trustwave"]
        },
        {
            "date": "2025-03",
            "name": "GitHub Copilot & Cursor Code-Agent Exploit",
            "impact": "Manipulated AI code suggestions injected backdoors, leaked API keys, and introduced logic flaws into production code.",
            "asi_mappings": ["ASI04", "ASI08", "ASI09"],
            "references": ["Pillar Security"]
        },
        {
            "date": "2025-02",
            "name": "OpenAI ChatGPT Operator Vulnerability",
            "impact": "Prompt injection in web content caused the Operator to follow attacker instructions and expose users' private data.",
            "asi_mappings": ["ASI01", "ASI02", "ASI03", "ASI04", "ASI06", "ASI07", "ASI09"],
            "references": ["Wunderwuzzi"]
        }
    ]
...
def create_simplified_version(data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Creates a simplified version with just IDs, titles, and key info for quick reference.
    """
    simplified = {
    ...
    for entry in data["entries"]:
        simplified["entries"].append({
            "id": entry["id"],
            "title": entry["title"],
            "description": entry["description"][:200] + "..." if len(entry["description"]) > 200 else entry["description"],
            "related_llm_entries": entry["related_llm_entries"],
            "aivss_core_risk": entry["aivss_core_risk"],
            "mitigation_count": len(entry["mitigations"]),
            "attack_scenario_count": len(entry["attack_scenarios"])
        })
...
def main():
    ...
    print("=" * 60)
    print("OWASP Top 10 for Agentic Applications 2026 - Parser")
    print("=" * 60)
    print()
    
    # Create the main data structure
    print("📦 Creating OWASP Agentic Top 10 data structure...")
    data = create_owasp_agentic_top10()
    
    # Add exploits tracker
    data["exploits_tracker"] = {
        "description": "ASI Agentic Exploits & Incidents Tracker - Real-world incidents mapped to ASI entries",
        "incidents": create_exploits_tracker()
    }
    
    # Save full JSON
    save_json(data, "owasp_agentic_top10_full.json")
    
    # Save full YAML
    save_yaml(data, "owasp_agentic_top10_full.yaml")
    
    # Create and save simplified version
    print("📦 Creating simplified reference version...")
    simplified = create_simplified_version(data)
    save_json(simplified, "owasp_agentic_top10_simplified.json")
    save_yaml(simplified, "owasp_agentic_top10_simplified.yaml")
    
    # Create entries-only version for easy import
    print("📦 Creating entries-only version...")
    entries_only = {"entries": data["entries"]}
    save_json(entries_only, "owasp_agentic_top10_entries.json")
    save_yaml(entries_only, "owasp_agentic_top10_entries.yaml")
    
    # Create mappings-only version
    print("📦 Creating mappings-only version...")
    mappings_only = {
        "asi_llm_mappings": data["mappings"],
        "nhi_mappings": data["nhi_mappings"]
    }
    save_json(mappings_only, "owasp_agentic_top10_mappings.json")
    save_yaml(mappings_only, "owasp_agentic_top10_mappings.yaml")
    ...
    print("  📄 owasp_agentic_top10_full.json       - Complete data structure")
    print("  📄 owasp_agentic_top10_full.yaml       - Complete data structure")
    print("  📄 owasp_agentic_top10_simplified.json - Quick reference version")
    print("  📄 owasp_agentic_top10_simplified.yaml - Quick reference version")
    print("  📄 owasp_agentic_top10_entries.json    - Entries only")
    print("  📄 owasp_agentic_top10_entries.yaml    - Entries only")
    print("  📄 owasp_agentic_top10_mappings.json   - Framework mappings")
    print("  📄 owasp_agentic_top10_mappings.yaml   - Framework mappings")
    print()
    
    # Print summary statistics
    print("📊 Summary Statistics:")
    print(f"   Total ASI Entries: {len(data['entries'])}")
    total_mitigations = sum(len(e['mitigations']) for e in data['entries'])
    total_scenarios = sum(len(e['attack_scenarios']) for e in data['entries'])
    total_examples = sum(len(e['common_examples']) for e in data['entries'])
    print(f"   Total Mitigations: {total_mitigations}")
    print(f"   Total Attack Scenarios: {total_scenarios}")
    print(f"   Total Common Examples: {total_examples}")
    print(f"   Total Tracked Incidents: {len(data['exploits_tracker']['incidents'])}")
    print(f"   NHI Mappings: {len(data['nhi_mappings']['mappings'])}")
